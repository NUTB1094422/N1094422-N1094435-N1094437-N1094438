...
class Player(pygame.sprite.Sprite):            #飛船
    ...
    def update(self): 
        ...
        if key_pressed[pygame.K_RIGHT]:            #移動鍵為右鍵
            self.rect.x += self.speedx
        if key_pressed[pygame.K_LEFT]:             #移動鍵為左鍵
            self.rect.x -= self.speedx
...
class Explosion(pygame.sprite.Sprite):              #子彈
...
class Power(pygame.sprite.Sprite):              #子彈
    def __init__(self, center):
        pygame.sprite.Sprite.__init__(self)
        self.type = random.choice(["shield", "gun"])
        self.image = power_imgs[self.type]   #image顯示的圖片
        self.image.set_colorkey(BLACK)
        self.rect = self.image.get_rect()      #rect定位圖片 可以設定X,Y座標
        self.rect.center = center
        self.speedy = 3  #垂直移動速度
